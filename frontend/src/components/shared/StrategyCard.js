...
export default function StrategyCard({ title, score, strategy, metrics = {}, className, children }) {
  return (
    <div className={cn('bg-terminal-bg-card border border-terminal-border rounded-xl p-5 hover:border-terminal-green/30 transition-colors', className)}>
      <div className="flex items-start justify-between mb-3">
        <div>
          ...
          {strategy && (
            <span className="inline-block mt-1 px-2 py-0.5 text-xs font-mono rounded bg-terminal-green/10 text-terminal-green border border-terminal-green/20 uppercase tracking-wider">
              {strategy}
            </span>
          )}
        </div>
